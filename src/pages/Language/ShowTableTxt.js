...
const ShowTableTxt = ({ signsupper, signslower, title, showUni }) => {
    ...
    return (<>
        ...
        {signsupper[0] && <div><p>{String.fromCharCode('0x000A')}Upper</p>{String.fromCharCode('0x000A')}
            {signsupper.map(item => {
        ...
        {signslower[0] && <div><p>{String.fromCharCode('0x000A')}Lower</p>{String.fromCharCode('0x000A')}
            {signslower.map(item => {
                if (item.alts && item.alts[0]) {
                    return (<>{singleLetter(item)}{singleLetterMap(item.alts)}</>)
                } else {
